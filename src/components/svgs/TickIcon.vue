<template>
<svg :class="[fill? `fill-${fill}` : '']" enable-background="new 0 0 2000 2000" height="512" viewBox="0 0 2000 2000" width="512" xmlns="http://www.w3.org/2000/svg"><g><g><path d="m698 1587c-17 0-33.3-6.7-45.3-18.7l-442-442c-25-25-25-65.5 0-90.5s65.5-25 90.5 0l396.8 396.7 1000.7-1000.8c25-25 65.5-25 90.5 0s25 65.5 0 90.5l-1046 1046c-12 12.1-28.2 18.8-45.2 18.8z"/></g></g></svg>
</template>

<script>

import fill from "./utilities/fill";
export default {
	name: "tick-icon",
	mixins: [fill]
};
</script>

<style>

</style>
